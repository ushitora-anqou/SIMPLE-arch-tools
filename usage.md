...
- 雰囲気を掴むために`examples/fiblt55.s`をアセンブルしてみる。
    ...
    - `cat ../examples/fiblt55.s | ./macro | ./assembler | ./emulator`とするとエミュレータがこれを実行する。今回の場合であれば55より小さいフィボナッチ数列が表示される。また`$?`は55となる。
...
そのバイナリファイルを`emulator`のコマンドライン第二引数に渡して実行する。
このとき`emulator`の出力にはメモリダンプが含まれる。

`debugger`は現状この機能に対応していない（TODO）。
