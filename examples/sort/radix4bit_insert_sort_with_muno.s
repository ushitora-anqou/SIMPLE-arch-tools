...
// scan forward
inline scan_forward() {
	alloc src, dst, limit, tmp, tmp2, i16

	src = 0x04
	src <<= 8	// src == 0x0400
	limit = 0x08
	limit <<= 8	// limit == 0x0800
	dst = 0x28
	dst <<= 8	// dst == 0x2800
	i16 = 1
	i16 <<= 4	// i16 == 16
	...
	[dst] = tmp
loop:
	<% 15.times do |index_minus_1| %>
		<% index = index_minus_1 + 1 %>
		<% if index.odd? %>
			tmp2 = [src + <%= index %>]
			if tmp2 < tmp then goto exit
			[dst + <%= index %>] = tmp2
		<% else %>
			tmp = [src + <%= index %>]
			if tmp < tmp2 then goto exit
			[dst + <%= index %>] = tmp
		<% end %>
	<% end %>
	...
	src += i16
	dst += i16
	if limit <= src then goto letshalt
	tmp = [src]
	if tmp < tmp2 then goto exit	// not sorted data
	[dst] = tmp
...
loop:
	...
	if limit <= src then goto letshalt
	tmp = [src]
	if tmp2 < tmp then goto exit	// not sorted data
	[dst] = tmp
